...
start = 0
min_len = min(len(acc), len(acc_nog), len(gyr), len(mag), start + 100000000)
...
for i, t in enumerate(timestamps):
    ...
    fuel_use = nav.get_fuel(return_totals=True)
    fuel.append(fuel_use['fuel_current'])
    tfuel.append(fuel_use['fuel_total'])

    emissions = nav.get_emissions(return_totals=True)
    co2.append(emissions['co2_current'])
    tco2.append(emissions['co2_total'])
    co.append(emissions['co_current'])
    tco.append(emissions['co_total'])
    nox.append(emissions['nox_current'])
    tnox.append(emissions['nox_total'])
    particulate.append(emissions['particulate_current'])
    tparticulate.append(emissions['particulate_total'])
